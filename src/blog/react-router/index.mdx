...
that react router provides. Our router context is going to provide 2 keys;
location and push:
...
'popstate' event to update our location when using the browser back button.
...
small but crucial piece. Every good router needs a way to extract paramaters
